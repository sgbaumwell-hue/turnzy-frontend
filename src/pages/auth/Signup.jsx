...
export function Signup() {
  ...
  async function handleSubmit(e) {
    ...
    setLoading(true);
    try {
      const { data } = await authApi.signup(name, email, password, 'host');
      ...
      <div className="w-full max-w-sm">
        <Card className="rounded-2xl border-warm-200 shadow-sm">
          <CardContent className="p-8">
            ...
            <h1 className="text-center text-lg font-bold text-warm-800 mb-1">Create your account</h1>
            <p className="text-center text-xs text-warm-400 mb-6">Manage your properties and cleaners on Turnzy.</p>
            <Button variant="outline" fullWidth asChild className="mb-4 h-11 gap-3 text-warm-700 border-warm-200 hover:bg-warm-50">
              <a href={`${BACKEND_URL}/auth/google?role=host`}>
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              <Button type="submit" fullWidth loading={loading} className="mt-1">Create account</Button>
            </form>
