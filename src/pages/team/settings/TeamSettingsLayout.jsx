...
const SECTIONS = [
  { to: '/team/settings/notifications', icon: Bell, label: 'Notifications', subtitle: 'Alerts, timing, channels' },
  { to: '/team/settings/account', icon: User, label: 'Account', subtitle: 'Name, email, password' },
];
...
export function TeamSettingsLayout() {
  ...
  const navigate = useNavigate();
  const isRoot = location.pathname === '/team/settings' || location.pathname === '/team/settings/';

  if (isDesktop) {
    if (isRoot) return <Navigate to="/team/settings/notifications" replace />;
    return (
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-gray-50">
      <div className="sticky top-0 z-10 bg-white border-b border-gray-100 px-4 py-3 flex items-center gap-3">
        <button onClick={() => navigate('/team/settings')} className="p-1 -ml-1">
